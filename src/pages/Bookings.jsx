...
const Bookings = ({ user }) => {
  ...
  // ১. ব্যাকএন্ড থেকে ইউজারের সব সেশন ও বুকিং রিকোয়েস্ট লোড করা
  useEffect(() => {
  ...
  const handleAccept = async (bookingId) => {
    ...
    if (!link) {
      alert('দয়া করে Google Meet লিংকটি প্রবেশ করান!');
      return;
    }
  ...
  return (
    <div className="container py-4">
      <h2 className="fw-bold mb-1 text-dark">My Bookings & Sessions</h2>
      <p className="text-secondary mb-4">আপনার আগামীর সব সেশন এবং পেন্ডিং রিকোয়েস্টগুলো এখান থেকে ম্যানেজ করুন।</p>
                  ...
                  <div className="card-body d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                      <div className="d-flex align-items-center gap-2 mb-2">
                        <h5 className="fw-bold mb-0 text-dark">{partner?.name || 'User'}</h5>
                        <span className={`badge ${booking.bookingType === 'FREE_SWAP' ? 'bg-success-subtle text-success border border-success-subtle' : 'bg-primary-subtle text-primary border border-primary-subtle'} rounded-pill`}>
                          {booking.bookingType === 'FREE_SWAP' ? '⚡ Free Swap' : '🎓 Paid Mentorship'}
                        </span>
                        <span className={`badge ${booking.status === 'ACCEPTED' ? 'bg-success' : booking.status === 'PENDING' ? 'bg-warning text-dark' : 'bg-danger'}`}>
                          {booking.status}
                        </span>
                      </div>
                    ...
                    {/* অ্যাকশন বাটন সেকশন */}
                    <div className="d-flex flex-column gap-2 min-w-200">
                      {booking.status === 'PENDING' && isProvider && (
                        <div className="d-flex flex-column gap-2">
                          ...
                          <div className="d-flex gap-2">
                            <button onClick={() => handleAccept(booking._id)} className="btn btn-success btn-sm w-100 fw-semibold">
                              Accept
                            </button>
                            <button onClick={() => handleReject(booking._id)} className="btn btn-outline-danger btn-sm w-100">
                              Reject
                            </button>
                          </div>
                        </div>
                      ...
                      {booking.status === 'ACCEPTED' && booking.meetLink && (
                        <a href={booking.meetLink} target="_blank" rel="noopener noreferrer" className="btn btn-primary btn-sm fw-semibold rounded-pill px-3 text-center">
                          Join Google Meet 🚀
                        </a>
                      )}

                      {booking.status === 'PENDING' && !isProvider && (
                        <span className="text-muted small fst-italic">Waiting for provider approval...</span>
                      )}
                    </div>
                  </div>
                </div>
